...
public class FinderPattern {
	...
	static Point[] getCenter(Line[] crossLines) 
		...
		if (foundPoints.length == 3 || foundPoints.length == 6) {
			if(foundPoints.length == 6) {
				Point[] foundPoints2 = new Point[3];
				foundPoints2[0] = foundPoints[0];
				foundPoints2[1] = foundPoints[1];
				foundPoints2[2] = foundPoints[5];
				canvas.drawPolygon(foundPoints2, Color.RED);	
				return foundPoints2;
			}
//			Stream.of(foundPoints).forEach(fp -> System.out.println(fp));
//			foundPoints = Arrays.copyOfRange(foundPoints, 0, 3);
